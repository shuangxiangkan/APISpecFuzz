...
int main() {
	...
	while (__AFL_LOOP(1000)) 
	{
		...
		cJSON* result = cJSON_ParseWithLengthOpts(value, buffer_length, return_parse_end, require_null_terminated);

		std::string fileName = "cJSON_ParseWithLengthOpts.cpp";
		std::string funSignature = "cJSON* cJSON_ParseWithLengthOpts(char* value, size_t buffer_length, char** return_parse_end, cJSON_bool require_null_terminated)";
