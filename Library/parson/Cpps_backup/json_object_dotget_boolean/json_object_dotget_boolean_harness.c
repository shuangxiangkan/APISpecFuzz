...
int main() {
	...
	while (__AFL_LOOP(1000)) 
	{
		...
		const char *funSignature = "int json_object_dotget_boolean(JSON_Object* object, char* name)";
		
		if(object != NULL && *object -> names == name)
		{
			SpecFileGeneration("*object -> names == name;", "json_object_dotget_boolean_0.cpp", funSignature);
		}
