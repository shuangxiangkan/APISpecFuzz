...
int main() {
	...
	while (__AFL_LOOP(1000)) 
	{
		...
		cJSON* result = cJSON_AddNumberToObject(object, name, number);

		std::string fileName = "cJSON_AddNumberToObject.cpp";
		std::string funSignature = "cJSON* cJSON_AddNumberToObject(cJSON* object, char* name, double number)";
		...
		if(result != NULL &&result -> valuedouble == number)
		{
			SpecFileGeneration("result -> valuedouble = number", fileName, funSignature);
		}
