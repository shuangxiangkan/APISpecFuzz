...
int main(int argc, char *argv[])
{
  char buf[1024];
    ...
    char *s_pTest_str = strtok(buf, "\n");
    char *s_pComment = strtok(NULL, "\n");
    ...
    for (i = (N - 1); i >= 0; --i)
    {
      ...
      if (s_Test_archive_filename == archive_filename) 
      {
        SpecFileGeneration("pZip_filename = pArchive_name;", "mz_zip_add_mem_to_archive_file_in_place_0.c", "MINIZ_EXPORT mz_bool mz_zip_add_mem_to_archive_file_in_place(const char *pZip_filename, const char *pArchive_name, const void *pBuf, size_t buf_size, const void *pComment, mz_uint16 comment_size, mz_uint level_and_flags)");
      }
      if (data == s_pComment) 
      {
        SpecFileGeneration("pBuf == pComment;", "mz_zip_add_mem_to_archive_file_in_place_1.c", "MINIZ_EXPORT mz_bool mz_zip_add_mem_to_archive_file_in_place(const char *pZip_filename, const char *pArchive_name, const void *pBuf, size_t buf_size, const void *pComment, mz_uint16 comment_size, mz_uint level_and_flags)");
      }
    }
    ...
    if (zip_archive.m_pAlloc_opaque == s_Test_archive_filename) 
    {
      SpecFileGeneration("pZip->m_pAlloc_opaque = pFilename;", "mz_zip_reader_init_file_0.cpp", "MINIZ_EXPORT mz_bool mz_zip_reader_init_file(mz_zip_archive *pZip, const char *pFilename, mz_uint32 flags)");
    }
    if (zip_archive.m_pIO_opaque == s_pComment) 
    {
      SpecFileGeneration("pZip->m_pIO_opaque = pFree;", "mz_zip_reader_init_file_1.cpp", "MINIZ_EXPORT mz_bool mz_zip_reader_init_file(mz_zip_archive *pZip, const char *pFilename, mz_uint32 flags)");
    }
    ...
    for (sort_iter = 0; sort_iter < 2; sort_iter++)
    {
      ...
      for (i = 0; i < N; i++)
      {
        ...
        if (zip_archive.m_pAlloc_opaque == archive_filename) 
        {
          SpecFileGeneration("pZip->m_pAlloc_opaque = pFilename;", "mz_zip_reader_extract_file_to_heap_0.c", "void *mz_zip_reader_extract_file_to_heap(mz_zip_archive *pZip, const char *pFilename, size_t *pSize, mz_uint flags)");
        }
        if (zip_archive.m_pIO_opaque == archive_filename) 
        {
          SpecFileGeneration("pZip->m_pIO_opaque = pFilename;", "mz_zip_reader_extract_file_to_heap_1.cpp", "void *mz_zip_reader_extract_file_to_heap(mz_zip_archive *pZip, const char *pFilename, size_t *pSize, mz_uint flags)");
        }
        if (zip_archive.m_archive_size == uncomp_size) 
        {
          SpecFileGeneration("pZip->m_archive_size = *pSize;", "mz_zip_reader_extract_file_to_heap_2.cpp", "void *mz_zip_reader_extract_file_to_heap(mz_zip_archive *pZip, const char *pFilename, size_t *pSize, mz_uint flags)");
        }
