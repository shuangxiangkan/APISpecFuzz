...
# 创建文件夹
string_header_path = "utf8_2.h"  # 使用相对路径
...
default_headers_code = ('#include "utf8.h"\n'
                        '#include <string.h>\n'
                        '#include <stdio.h>\n\n')
...
def createTestCasesinInFolder(Parms, cpp_harness_in_folder_path):
    ...
    while iterater < 1:
        ...
        for i, param_decl in enumerate(Parms):
            ...
            if argInfo[0] == "unsigned" or argInfo[0] == "unsigned int" or argInfo[0] == "utf8_int32_t":
                if i == 0:
                    with open(cpp_test_case_file_name, "a") as cpp_test_case_file:
                        cpp_test_case_file.write(f"{random_string_length}\n")
                elif i == 1:
            ...
            elif argInfo[0] == "char" or argInfo[0] == "utf8_int8_t":
                if argInfo[1] > 0:
                    if i == 0:
                        with open(cpp_test_case_file_name, "a") as cpp_test_case_file:
...
excluded_functions = ["utf8dup_ex", "utf8ndup_ex"]
class FuncDeclVisitor(NodeVisitor):
    ...
    def visit_FuncDecl(self, node):
        ...
        for i, param_decl in enumerate(node.args.params):
            ...
            if argInfo[0] == "int" or argInfo[0] == "cJSON_bool" or argInfo[0] == "utf8_int32_t":
                cpp_code += f"\tint "
                if argInfo[1] > 0:
            ...
            if argInfo[0] == "unsigned" or argInfo[0] == "unsigned int":
                ...
                if i == 0:
                    ...
                    for j in range(argInfo[1]):
                        cpp_code += "*"
                    cpp_code += f"{getParamName(param_decl)} = cJSON_Parse(strtok({buf}, \"\\n\"));\n\t"
                else:
                    cpp_code += f"\tcJSON "
                    for j in range(argInfo[1]):
                        cpp_code += "*"
                    cpp_code += f"{getParamName(param_decl)} = cJSON_Parse(strtok(NULL, \"\\n\"));\n\t"
