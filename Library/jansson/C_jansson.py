...
# 创建文件夹
string_header_path = "jansson.h"  # 使用相对路径
...
default_headers_code = ('#include "jansson.h"\n'
                        '#include <string.h>\n'
                        '#include <stdio.h>\n\n')
...
typydef_names = {"JSON_Object", "JSON_Array", "JSON_Value", "JSON_Value_Type", "JSON_Status", "JSON_String"}
struct_map = {}
        ...
        if key == node:
            ...
            for field in value["fields"]:
                ...
                if isinstance(field['type'], PtrDecl) and isinstance(field['type'].type.type, Struct) and field[
                    ...
                    name_path.append(field['field'])
                    print("field['type'].type: ", field['type'].type)
                    if isinstance(field['type'].type, IdentifierType):
                        type_path.append(field['type'].type.names[0])
                    elif isinstance(field['type'].type, PtrDecl):
                        type_path.append(field['type'].type.type.type.names[0])
                    pt = Points_to_Information(getPointerLevel(field['type'], 0), copy.copy(name_path),
                                               copy.copy(type_path))
                    points_to_path[variable_name].append(pt)
                    name_path.pop()
...
def createJsonTestCases():
    ...
    # 将所有 JSON 字符串放入列表中
    json_strings = [json_string1, json_string2, json_string3]

    # 随机选择并返回一个 JSON 字符串
    return random.choice(json_strings)

def createJsonArrayTestCases():
    json_string1 = '[1, 2, 3, 4, 5]'
    json_string2 = '["name", "Banana", "age", 30, 3.14]'
    json_string3 = '[{"name": "John", "age": 30, "isMarried": false, "hobbies": ["Reading", "Cycling"]}, {"name": "Jane Smith", "age": 28, "isMarried": true, "hobbies": ["Painting", "Dancing"], "children": [{"name": "Alice", "age": 5}, {"name": "Bob", "age": 3}]}]'
            ...
            if argInfo[0] == "int":
                if i == 0:
                    with open(cpp_test_case_file_name, "a") as cpp_test_case_file:
                        cpp_test_case_file.write(f"{createIntSize_t()}\n")
                elif i == 1:
                    with open(cpp_test_case_file_name, "a") as cpp_test_case_file:
                        cpp_test_case_file.write(f"{createIntSize_t()}\n")
                else:
                    with open(cpp_test_case_file_name, "a") as cpp_test_case_file:
                        cpp_test_case_file.write(f"{createIntSize_t()}\n")
            if argInfo[0] == "unsigned" or argInfo[0] == "unsigned int" or argInfo[0] == "JSON_Value_Type":
                if i == 0:
                    with open(cpp_test_case_file_name, "a") as cpp_test_case_file:
                        cpp_test_case_file.write(f"{createIntSize_t()}\n")
                elif i == 1:
                ...
                if argInfo[1] > 0:
                    if i == 0:
                        ...
                        with open(cpp_test_case_file_name, "a") as cpp_test_case_file:
                            cpp_test_case_file.write(f"{substring}\n")
            elif argInfo[0] == "JSON_Value" or argInfo[0] == "json_t":
                if argInfo[1] > 0:
                    if i == 0:
                        with open(cpp_test_case_file_name, "a") as cpp_test_case_file:
                        ...
                        with open(cpp_test_case_file_name, "a") as cpp_test_case_file:
                            cpp_test_case_file.write(f"{createJsonTestCases()}\n")
            elif argInfo[0] == "JSON_Array":
                if argInfo[1] > 0:
                    if i == 0:
                        with open(cpp_test_case_file_name, "a") as cpp_test_case_file:
                            cpp_test_case_file.write(f"{createJsonArrayTestCases()}\n")
                    else:
                        with open(cpp_test_case_file_name, "a") as cpp_test_case_file:
                            cpp_test_case_file.write(f"{createJsonArrayTestCases()}\n")
            elif argInfo[0] == "JSON_Object":
                if argInfo[1] > 0:
                    if i == 0:
                        with open(cpp_test_case_file_name, "a") as cpp_test_case_file:
                            cpp_test_case_file.write(f"{createJsonTestCases()}\n")
...
excluded_functions = ["json_dump_callback","json_load_callback"]
class FuncDeclVisitor(NodeVisitor):
    ...
    def visit_FuncDecl(self, node):
        ...
        for i, param_decl in enumerate(node.args.params):
            ...
            if argInfo[0] == "int" or argInfo[0] == "cJSON_bool":
                ...
                if argInfo[1] > 0:
                    for j in range(argInfo[1]):
                        cpp_code += "*"
                cpp_code += f"{getParamName(param_decl)};\n\t"
                cpp_code += f"\tsscanf(strtok(NULL, \"\\n\"), \"%d\", &{getParamName(param_decl)});\n\n\t"
            ...
            if argInfo[0] == "unsigned" or argInfo[0] == "unsigned int" or argInfo[0] == "JSON_Value_Type":
                ...
                if argInfo[1] > 0:
                    for j in range(argInfo[1]):
                        cpp_code += "*"
                    cpp_code += f"{getParamName(param_decl)} = strtok({buf}, \"\\n\");\n\t"
                else:
                    cpp_code += f"{getParamName(param_decl)} = strtok({buf}, \"\\n\")[0];\n\t"
            elif argInfo[0] == "void":
                if argInfo[1] > 0:
                    ...
                    for j in range(argInfo[1]):
                        cpp_code += "*"
                    cpp_code += f"{getParamName(param_decl)} = strtok({buf}, \"\\n\");\n\t"
            elif argInfo[0] == "JSON_Value":
                if i == 0:
                    cpp_code += f"\tJSON_Value "
                    for j in range(argInfo[1]):
                        cpp_code += "*"
                    cpp_code += f"{getParamName(param_decl)} = json_parse_string(strtok({buf}, \"\\n\"));\n\t"
                else:
                    cpp_code += f"\tJSON_Value "
                    for j in range(argInfo[1]):
                        cpp_code += "*"
                    cpp_code += f"\t{getParamName(param_decl)} = json_parse_string(strtok(NULL, \"\\n\"));\n\t"
                cJson_objects_used.append(getParamName(param_decl))

            elif argInfo[0] == "JSON_Array":
                if i == 0:
                    cpp_code += f"\tJSON_Value *{temp + str(temp_number)} = json_parse_string(strtok({buf}, \"\\n\"));\n\t"
                    cpp_code += f"\tJSON_Array "
                    for j in range(argInfo[1]):
                        cpp_code += "*"
                    cpp_code += f"{getParamName(param_decl)} = json_array({temp + str(temp_number)});\n\t"
                else:
                    cpp_code += f"\tJSON_Value *{temp + str(temp_number)} = json_parse_string(strtok({buf}, \"\\n\"));\n\t"
                    cpp_code += f"\tJSON_Array "
                    for j in range(argInfo[1]):
                        cpp_code += "*"
                    cpp_code += f"{getParamName(param_decl)} = json_array({temp + str(temp_number)});\n\t"
                cJson_objects_used.append(temp + str(temp_number))
                temp_number = temp_number + 1

            elif argInfo[0] == "JSON_Object":
                if i == 0:
                    cpp_code += f"\tJSON_Value *{temp + str(temp_number)} = json_parse_string(strtok({buf}, \"\\n\"));\n\t"
                    cpp_code += f"\tJSON_Object "
                    for j in range(argInfo[1]):
                        cpp_code += "*"
                    cpp_code += f"{getParamName(param_decl)} = json_object({temp + str(temp_number)});\n\t"
                else:
                    cpp_code += f"\tJSON_Value *{temp + str(temp_number)} = json_parse_string(strtok({buf}, \"\\n\"));\n\t"
                    cpp_code += f"\tJSON_Object "
                    for j in range(argInfo[1]):
            ...
            elif argInfo[0] == "json_t":
                if json_error_t_flag == False:
                    cpp_code += f"\tjson_error_t error;\n\t"
                    json_error_t_flag = True
                if i == 0:
                    cpp_code += f"\tjson_t "
                    for j in range(argInfo[1]):
                        cpp_code += "*"
                    cpp_code += f"{getParamName(param_decl)} = json_loads(strtok({buf}, \"\\n\"), 0, &error);\n\t"
                else:
                    cpp_code += f"\tjson_t "
                    for j in range(argInfo[1]):
                        cpp_code += "*"
                    cpp_code += f"{getParamName(param_decl)} = json_loads(strtok(NULL, \"\\n\"), 0, &error);\n\t"
            ...
            if len(node.args.params) == 1:
                ...
                if argInfo[1] > 0:
                    for i in range(argInfo[1]):
                        fun_signature_code += "*"
                        # print("*", end="")
                fun_signature_code += f" {getParamName(param_decl)}, "
        ...
        if funcNameInfo[0] == "json_object_get_value":
        ...
        for i, param_decl in enumerate(node.args.params):
            ...
            if argInfo[1] > 0 and argInfo[0] in struct_map:
                type_path = [argInfo[0]]
                name_path = [getParamName(param_decl)]
                pt = Points_to_Information(argInfo[1], name_path, type_path)
                points_to_path[getParamName(param_decl)] = [pt]
                # ExtendStructPointer(param_decl.name, argInfo[0], name_path, type_path, points_to_path)
                print("param_decl.name: ", param_decl.name)
                ExtendStructPointer(getParamName(param_decl), argInfo[0], name_path, type_path, points_to_path)
            else:
                type_path = [argInfo[0]]
                name_path = [getParamName(param_decl)]
                points_to_path[getParamName(param_decl)] = []
                pt = Points_to_Information(argInfo[1], name_path, type_path)
                points_to_path[getParamName(param_decl)].append(pt)
