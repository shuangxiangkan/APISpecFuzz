...
def compile_specification_IR_files_in_directory(directory):
...
def compile_complete_IR_files_in_directory(directory):
    for root, _, files in os.walk(directory):
        for filename in files:
            if filename.endswith(".c"):
                source_file = os.path.join(root, filename)
...
def compile_ll_files_in_directory(directory):
    for root, _, files in os.walk(directory):
        for filename in files:
            if filename.endswith(".ll") and not filename.endswith("_specification.ll"):
                source_file = os.path.join(root, filename)
...
compile_specification_IR_files_in_directory(current_directory)
# 调用函数来递归遍历并编译
compile_complete_IR_files_in_directory(current_directory)
