...
int main() {
	...
	while (__AFL_LOOP(1000)) 
	{
		...
		cJSON* result = cJSON_AddRawToObject(object, name, raw);

		std::string fileName = "cJSON_AddRawToObject.cpp";
		std::string funSignature = "cJSON* cJSON_AddRawToObject(cJSON* object, char* name, char* raw)";
		...
		if(object != NULL &&object -> valuestring == raw)
		{
			SpecFileGeneration("object -> valuestring = raw", fileName, funSignature);
		}
		if(object != NULL &&object -> string == raw)
		{
			SpecFileGeneration("object -> string = raw", fileName, funSignature);
		}
		if(name == raw)
		{
			SpecFileGeneration("name = raw", fileName, funSignature);
		}
	}
