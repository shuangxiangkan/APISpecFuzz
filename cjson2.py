...
spec_files_dir_name = "Cpps/"
# spec_file_name = ""
spec_file_suffix_name = ".cpp"
...
def SpecFileGenFunctionCode():
    ...
    code += "{\n"
    code += f"\tstd::string fileName = fileName;"
...
def getSpecCode(funcNameInfo, funSignature, lvalue, lvaluePointerLevel, rvalue, rvaluePointerLevel):
    spec_code = f"\n\t\t\tstd::string fileName = \"{funcNameInfo}.cpp\";"
    spec_code += f"\n\t\t\tstd::ofstream file(fileName, std::ios::app);"
    spec_code += f"\n\t\t\tfile << \"{funSignature}\\n\";"
    spec_code += f"\n\t\t\tfile << \"{{\\n\";"
    ...
    if lvalue == "result":
        spec_code += f"\n\t\t\tfile << \"\treturn {rvalue};\\n\";"
    elif lvaluePointerLevel == rvaluePointerLevel + 1:
        spec_code += f"\n\t\t\tfile << \"\t*{lvalue} = {rvalue};\\n\";"
    elif lvaluePointerLevel + 1 == rvaluePointerLevel:
        spec_code += f"\n\t\t\tfile << \"\t{lvalue} = *{rvalue};\\n\";"
    elif lvaluePointerLevel == rvaluePointerLevel:
        spec_code += f"\n\t\t\tfile << \"\t{lvalue} = {rvalue};\\n\";"
    ...
    def visit_FuncDecl(self, node):
        ...
        if funcNameInfo[0].startswith("cJSON_") and funcNameInfo[0] != "cJSON_InitHooks":
            ...
            for i, param_decl in enumerate(node.args.params):
                ...
                if argInfo[0] == "cJSON":
                    cpp_code += f"\tcJSON *{param_decl.name} = cJSON_CreateObject();\n\t"
                elif argInfo[0] == "int":
                    cpp_code += f"\tint {param_decl.name} = 0;\n\t"
                elif argInfo[0] == "cJSON_bool":
                    cpp_code += f"\tcJSON_bool {param_decl.name} = 0;\n\t"
                elif argInfo[0] == "float":
                    cpp_code += f"\tfloat {param_decl.name} = 0.0;\n\t"
                elif argInfo[0] == "double":
                    cpp_code += f"\tdouble {param_decl.name} = 0.0;\n\t"
                elif argInfo[0] == "char":
                    cpp_code += f"\tchar* {param_decl.name} = \"abc\";\n\t"
                ################### 参数初始化 ###################
            ...
            for key1, value1 in points_to_path.items():
                for key2, value2 in points_to_path.items():
                    if key1 != key2 and (key1, key2) not in processed_pairs and (key2, key1) not in processed_pairs:
                        ...
                        for pt1 in value1:
                            for pt2 in value2:
                                if pt1.type_path[-1] != "void" and pt2.type_path[-1] != "void" and pt1.type_path[-1] == \
                                        pt2.type_path[-1] and pt1.pointer_level == pt2.pointer_level:
                                    lvalue = ""
                                    ...
                                    cpp_code += "\n\t\t{"
                                    # spec_code = getSpecCode(funcNameInfo[0], fun_signature_code, lvalue, pt1.pointer_level, rvalue, pt2.pointer_level)

                                    spec_code = f"\n\t\t\tSpecFileGeneration(\"{lvalue} = {rvalue}\", fileName, funSignature);"


                                    # spec_code = f"\n\t\t\tstd::string fileName = \"{funcNameInfo[0]}.cpp\";"
                                    # spec_code += f"\n\t\t\tstd::ofstream file(fileName, std::ios::app);"
                                    # spec_code += f"\n\t\t\tfile << \"{fun_signature_code}\\n\";"
                                    # spec_code += f"\n\t\t\tfile << \"{{\\n\";"
                                    # spec_code += f"\n\t\t\tfile << \"\t{lvalue} = {rvalue};\\n\";"
                                    # spec_code += f"\n\t\t\tfile << \"}}\\n\";"
                                    # spec_code += f"\n\t\t\tfile.close();"
                                    cpp_code += spec_code
                                    cpp_code += "\n\t\t}"
                                    print(f" ################# copy: {lvalue} == {rvalue}")
                                elif pt1.type_path[-1] != "void" and pt2.type_path[-1] != "void" and pt1.type_path[
                                    -1] == pt2.type_path[-1] and pt1.pointer_level == pt2.pointer_level + 1:
                                    lvalue = ""
                                    for name in pt1.name_path:
                                        lvalue += name + "->"
                                    lvalue = lvalue[:-2]
                                    ...
                                    cpp_code += spec_code
                                    cpp_code += "\n\t\t}"
                                    print(f" ################# store: *{lvalue} == {rvalue}")
                                elif pt1.type_path[-1] != "void" and pt2.type_path[-1] != "void" and pt1.type_path[
                                    -1] == pt2.type_path[-1] and pt1.pointer_level + 1 == pt2.pointer_level:
                                    lvalue = ""
                                    for name in pt1.name_path:
                                        lvalue += name + "->"
                                    lvalue = lvalue[:-2]
