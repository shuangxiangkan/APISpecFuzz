...
int main() {
	...
	while (__AFL_LOOP(1000)) 
	{
		...
		cJSON* result = cJSON_DetachItemFromArray(array, which);

		std::string fileName = "cJSON_DetachItemFromArray.cpp";
		std::string funSignature = "cJSON* cJSON_DetachItemFromArray(cJSON* array, int which)";
		...
		if(result != NULL &&result -> type == which)
		{
			SpecFileGeneration("result -> type = which", fileName, funSignature);
		}
		if(result != NULL &&result -> valueint == which)
		{
			SpecFileGeneration("result -> valueint = which", fileName, funSignature);
		}
